...
class TimeStampedModel(models.Model):
    ...
    # auto_now_add creates the timestamp only when object is created.
    created_at = models.DateTimeField(auto_now_add=True)
    # auto_now updates the timestamp everytime the object is update.
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        # Model is not created in the database. It will only be used as a base class.
        abstract = True
...
class ContentView(TimeStampedModel):
    ...
    class Meta:
        ...
        # The combination of content_type, object_id, and user must be unique across all rows.
        unique_together = ["content_type", "object_id", "user", "viewer_ip"]

    def __str__(self) -> str:
        user_name = self.user.get_full_name() if self.user else "Anonymous"
        return f"{self.content_object} viewed by {user_name} from IP {self.viewer_ip}"
